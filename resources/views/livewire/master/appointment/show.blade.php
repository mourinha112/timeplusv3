                ...
                <div class="grid gap-6 md:grid-cols-[1fr_auto] items-start">
                    <div class="space-y-6">
                        <div class="flex items-center justify-between">
                            <div>
                                <x-text>Agendamento <span class="text-info font-bold">#{{ $appointment->id }}</span></x-text>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div class="p-4 bg-base-200/30 rounded-lg">
                                <x-text>Sala</x-text>
                                <div class="text-base font-medium">{{ $appointment->room?->code ?? '---' }}
                                    @if ($appointment->room)
                                        <span class="badge badge-sm {{ $appointment->room->status === 'open' ? 'badge-success' : 'badge-ghost' }}">
                                            {{ $appointment->room->status === 'open' ? 'Aberta' : 'Fechada' }}
                                        </span>
                                    @endif
                                </div>
                            </div>
                        ...
                        @if ($appointment->notes)
                            <div class="p-4 bg-base-200/20 rounded-lg border border-base-300">
                                <div class="font-semibold mb-2">Anotacoes</div>
                                ...
                                <div>
                                    ...
                                    <div class="text-base font-medium">
                                        @if ($appointment->payment)
                                            <a wire:navigate class="link link-info" href="{{ route('master.payment.show', ['payment' => $appointment->payment->id]) }}">#{{ $appointment->payment->id }}</a>
                                        @else
                                            ---
                                        @endif
                                    </div>
                    ...
                    <div class="flex flex-col gap-3 w-full md:w-56">
                        <a wire:navigate href="{{ route('master.appointment.index') }}" class="btn btn-soft btn-sm btn-info">
                            <x-carbon-arrow-left class="w-5 h-5" /> Voltar para a Lista
                        </a>
                        @if ($appointment->status === 'scheduled')
                            <button wire:click="cancelAppointment" wire:confirm="Tem certeza que deseja cancelar este agendamento?" class="btn btn-soft btn-sm btn-error">
                                <x-carbon-close class="w-5 h-5" /> Cancelar Agendamento
                            </button>
                        @endif
                    </div>
                </div>
