<div>
    <x-heading>
        <x-title class="flex items-center gap-3">
            <x-carbon-certificate class="w-8 text-info" />
            Editar Especialidade
        </x-title>
    </x-heading>

    <div class="flex justify-between items-center mb-6">
        <a wire:navigate href="{{ route('master.specialty.index') }}" class="btn btn-ghost">
            <x-carbon-arrow-left class="w-4 h-4" />
            Voltar para Especialidades
        </a>
    </div>

    <x-card class="w-full">
        <x-card-body>
            <x-form wire:submit="save">
                <div class="space-y-6">
                    <x-form-group>
                        <x-label required>Nome da Especialidade</x-label>
                        <x-input type="text" wire:model="name" placeholder="Ex: Psicologia, Nutricao..." />
                        @error('name') <div class="text-error text-sm mt-1">{{ $message }}</div> @enderror
                    </x-form-group>
                </div>

                <div class="flex justify-end gap-3 mt-8">
                    <a wire:navigate href="{{ route('master.specialty.index') }}" class="btn btn-ghost">
                        <x-carbon-close class="w-4 h-4" /> Cancelar
                    </a>
                    <x-button type="submit" color="info">
                        <x-carbon-save class="w-4 h-4" /> Atualizar Especialidade
                    </x-button>
                </div>
            </x-form>
        </x-card-body>
    </x-card>
</div>
