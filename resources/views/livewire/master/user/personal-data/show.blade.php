                ...
                <div class="grid gap-6 md:grid-cols-[1fr_auto] items-start">
                    <div class="space-y-4">
                        <div class="flex items-center gap-4">
                            <div class="avatar">
                                <div class="w-20 h-20 rounded-full">
                                    @if ($user->avatar)
                                        <img src="{{ asset('storage/' . $user->avatar) }}" alt="Avatar do usuário" class="w-full h-full object-cover" />
                                    @else
                                        <div class="bg-neutral text-neutral-content rounded-full w-full h-full flex items-center justify-center">
                                            <span class="text-2xl font-bold">{{ strtoupper(substr($user->name ?? 'U', 0, 1)) }}</span>
                                        </div>
                                    @endif
                                </div>
                            </div>
                            <div>
                                <p class="text-base-content/70">Usuário <span class="text-info font-bold">#{{ $user->id }}</span></p>
                                <h2 class="text-2xl font-bold text-base-content">{{ $user->name ?? 'Não informado' }}</h2>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                            ...
                            <div>
                                <x-text class="font-bold">{{ $user->email_verified_at ? 'Verificado' : 'Não verificado' }}</x-text>
                                <div class="mt-0.5 text-xs text-base-content/60">E-mail</div>
                            </div>
                            <div>
                                <div class="text-base font-medium text-base-content">{{ $user->created_at->format('d/m/Y') }}</div>
