<div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 bg-base-100 border-base-300 border mt-4">
        <x-stat title="Assinaturas Ativas" :value="$this->activeSubscriptions" />
        <x-stat title="Salas Abertas" :value="$this->openRooms" />
        <x-stat title="Taxa de Conclusão" value="{{ $this->completionRate }}%" />
        <x-stat title="Receita do Mês" value="R$ {{ number_format($this->monthlyRevenue, 2, ',', '.') }}" />
    </div>
