    ...
    <x-card>
        <x-card-body>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-action-link wire:navigate href="{{ route('company.plan.index') }}" icon="carbon-plan"
                    title="Gerenciar Planos" description="Configure os planos da sua empresa" icon-color="info" />

                <x-action-link wire:navigate href="{{ route('company.employee.create') }}" icon="carbon-user"
                    title="Adicionar Funcionários" description="Cadastre novos funcionários" icon-color="success" />

                <x-action-link wire:navigate href="{{ route('company.employee.index') }}" icon="carbon-user-multiple"
                    title="Gerenciar Funcionários" description="Visualize e gerencie sua equipe" icon-color="success" />

                <x-action-link wire:navigate href="{{ route('company.profile.show') }}" icon="carbon-settings"
                    title="Perfil Empresa" description="Ajuste as configurações da empresa" icon-color="warning" />

                {{-- TODO: Montar funcionalidade de relatório --}}
                {{-- <x-action-link wire:navigate href="#" icon="carbon-document" title="Relatórios"
